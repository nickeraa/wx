...
Page(t(t({
    ...
    saleslist: function(t) {
        wx.navigateTo({
            url: "/pages/vipsaless/index/index?vipcode=" + t.currentTarget.dataset.xf_vipcode
        });
    },
    checkinput: function() {
        "" == this.data.setype ? wx.showModal({
            title: "提示",
            content: "请选择查询方式！",
            showCancel: !1,
            success: function(t) {
        ...
        }) : "0" == this.data.setype ? this.listvipcode() : "1" == this.data.setype ? this.listuser() : this.listphone();
    },
    listphone: function() {
        if ("" == this.data.vipphone) return wx.showToast({
            title: "请输入客户手机号查询",
            icon: "none",
            duration: 2e3
        }), !1;
        if (!/^(14[0-9]|13[0-9]|15[0-9]|17[0-9]|18[0-9]|19[0-9]|16[0-9]|12[0-9]|11[0-9]|10[0-9])\d{8}$$/.test(this.data.vipphone)) return wx.showToast({
            title: "请输入正确的手机号",
            icon: "none",
            duration: 1e3
        }), !1;
    ...
    listvipcode: function() {
        if ("" == this.data.vipcode) return wx.showToast({
            title: "请输入客户卡号查询",
            icon: "none",
            duration: 2e3
        }), !1;
...
}, "onShow", function(t) {}), "getznumber", function(t) {
    wx.navigateTo({
        url: "/pages/listall/index/index?znumber=" + t.currentTarget.dataset.znumber
    });
}));