...
Page({
  ...
  onLoad: function (options) {
    var e = this;
    // 员工扫码进入时场景参数在 options.scene（URL编码），格式：vipcode_记录ID；
    // 直链/调试时走 options.vipcode / options.rowid
    var scene = '';
    if (options.scene) {
      scene = decodeURIComponent(options.scene);
    }
    var raw = (options.vipcode || scene || '').trim();
    var rowid = (options.rowid || '').trim();
    // 兼容 scene 里用 _ 或 | 分隔的「卡号_rowid」格式（scene 不能用 |，微信 40129 invalid scene）
    if (!rowid) {
      var sepIdx = raw.indexOf('_');
      if (sepIdx === -1) sepIdx = raw.indexOf('|');
      if (sepIdx > -1) {
        rowid = raw.slice(sepIdx + 1);
    ...
    e.setData({ vipcode: vipcode, rowid: rowid, memberCard: vipcode });
    e.queryPrize(vipcode, rowid);
  },

  // 按卡号（+rowid）查询中奖记录（acj_prize_day）
  queryPrize: function (vipcode, rowid) {
    var e = this;
    wx.request({
      url: app.globalData.api + 'wx_cj_prizeinfo_day.ashx',
      data: { vipcode: vipcode, rowid: rowid },
      dataType: 'json',
      success: function (res) {
        ...
        if (d.errcode === 0) {
          e.setData({
            ...
            prizeImg: cjimgDay + d.plu_id + '.png?t=' + Date.now(),
            vipName: d.vip_name || '',
