...
Page({
  data: {
    ...
    yguserid: "",
    iconList: [
      { icon: "pay", color: "black", badge: 50, name: "待付款" },
      { icon: "send", color: "black", badge: 1, name: "已付款" },
    ],
    gridCol: 2,
    sumdfk: 0,
    sumdfh: 0,
  ...
  tjtypes: function () {
    ...
    wx.request({
      ...
      success: function (e) {
        console.log(e.data),
          e.data.length > 0 &&
            t.setData({ sumdfk: e.data[0].SUMDFK, sumdfh: e.data[0].SUMDFH }),
          console.log(t.data.sumdfk);
      },
    });
  },
  seall: function () {
    wx.navigateTo({ url: "/pages/alldfk/index/index?title=0" });
  },
  setype: function (e) {
    "待付款" == e.currentTarget.dataset.name &&
      wx.navigateTo({ url: "/pages/alldfk/index/index?title=0" }),
      "已付款" == e.currentTarget.dataset.name &&
        wx.navigateTo({ url: "/pages/alldfk/index/index?title=1" });
  },
  mydj: function () {
  ...
  address: function () {
    wx.navigateTo({ url: "/pages/address/index/index" });
  },
  scj: function () {
    wx.navigateTo({ url: "/pages/scitem/index/index" });
  },
  onShow: function (t) {
    console.log(wx.getStorageSync("vipcode")),
      wx.getStorageSync("yguserid") &&
        this.setData({ yguserid: wx.getStorageSync("yguserid") }),
      console.log(wx.getStorageSync("vipcode")),
      wx.getStorageSync("vipcode")
        ...
        : this.setData({ wxuserid: wx.getStorageSync("wxuserid") }),
  ...
  onGetPhoneNumber: function (e) {
    ...
    "getPhoneNumber:ok" == a
      ...
      : wx.showModal({
          title: "提示",
          content:
            "会员卡领取失败！请重试，选择手机号，会员卡需要绑定您的手机号，才能领取喔~",
          showCancel: !1,
          success: function (e) {
            e.confirm;
          },
  ...
  deciyption: function (t, a, o) {
    ...
    console.log("步骤4根据秘钥解密手机号码sessionID：", t),
      wx.request({
        ...
        data: { sessionID: t, encryptedData: a, iv: o },
        header: { "content-type": "application/json" },
        success: function (t) {
          if (
            (console.log(t.data),
            n.setData({ phone: t.data.phoneNumber }),
  ...
  inphone: function () {
    ...
    wx.showLoading({ title: "正在加载" }),
      wx.request({
        ...
        data: { vip_id: wx.getStorageSync("vip_id"), phone: t.data.phone },
        header: { "content-type": "application/x-www-form-urlencoded" },
        dataType: "json",
        success: function (a) {
          wx.hideLoading(),
