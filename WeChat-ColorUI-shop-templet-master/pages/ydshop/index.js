...
Page({
  ...
  bindplay: function () {
    this.setData({
      isShow: !1
    }), this.videoContext.play(), console.log("play");
  },
  bindended: function () {
  ...
  onLoad: function (t) {
    console.log(t.xf_plu),
      ...
      (wx.setStorageSync("fx", t.fx),
        this.setData({
          fx: wx.getStorageSync("fx")
        }));
    var a = this;
    wx.getSystemInfo({
      success: function (t) {
        a.setData({
            videohight: parseInt(0.75 * t.windowWidth)
          }),
  ...
  setype: function (t) {
    ...
    console.log(i),
      ...
      (this.setData(
          a(
            a(a({}, "iconList[1].name", "点击收藏"), "iconList[1].color", "gray"),
            "iconList[1].icon",
            "favor"
          )
        ),
  ...
  onGetPhoneNumbergwc: function (t) {
    if (t.detail.errMsg != "getPhoneNumber:ok") {
      console.log("getPhoneNumber fail:", t.detail.errMsg);
      var msg = t.detail.errMsg.indexOf("user deny") >= 0 || t.detail.errMsg.indexOf("cancel") >= 0
        ? "请选择手机号，才能购买喔"
        : "授权太频繁或已受限，请稍后再试";
    ...
    wx.request({
        ...
        success: function (t) {
          if (t.data && t.data.phoneNumber) {
            wx.setStorageSync("wxuserid", t.data.phoneNumber);
            s.setData({ wxuserid: t.data.phoneNumber, stop: false });
            wx.hideLoading();
            s.gw();
          } else {
            wx.hideLoading();
            s.setData({ stop: false });
  ...
  deciyptionsc: function (r, e_data, iv_data) {
    wx.showLoading({ title: '加载中...' });
    var s = this;
    wx.request({
        url: e.globalData.api + "wx_getvipphone.ashx",
        data: { sessionID: r, encryptedData: e_data, iv: iv_data },
        header: { "content-type": "application/json" },
        timeout: 10000,
        success: function (t) {
          if (t.data && t.data.phoneNumber) {
      ...
      wx.request({
        ...
        success: function (t) {
          ...
          a.setData({
            ...
            t.data[0].TBIMAGES1 &&
            (a.setData({
                "swiperlist[0]": a.data.pict + t.data[0].XF_PLU + "//" + t.data[0].TBIMAGES1,
              }),
              console.log(a.data.swiperlist[0])),
            t.data[0].TBIMAGES2 &&
            a.setData({
              "swiperlist[1]": a.data.pict + t.data[0].XF_PLU + "//" + t.data[0].TBIMAGES2,
            }),
            t.data[0].TBIMAGES3 &&
            a.setData({
              "swiperlist[2]": a.data.pict + t.data[0].XF_PLU + "//" + t.data[0].TBIMAGES3,
            }),
